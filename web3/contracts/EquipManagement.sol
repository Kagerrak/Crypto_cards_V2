...
contract EquipManagement {
    ...
    function equip(
        ...
        if (tokenId > 10000) {
            ...
            _equipCompositeToken(characterTokenId, _caller, tokenId);
        } else {
    ...
    function _equipItem(
        ...
        if (equippedItem != 0) {
            ...
            if (equippedItem > 10000) {
                _unequipCompositeItem(characterTokenId, owner, equippedItem);
            } else {
                _unequipItem(characterTokenId, owner, equippedItem);
            }
        }
    ...
    function _equipCompositeToken(
        ...
        if (tokenType == CharData.TokenType.CompositeItem) {
            _equipCompositeItem(characterTokenId, owner, compositeTokenId);
        } else if (tokenType == CharData.TokenType.CompositeSkill) {
            _equipCompositeSkill(characterTokenId, owner, compositeTokenId);
        }
    }
    ...
    function _equipCompositeItem(
        ...
        // Burn the composite token from the owner
        compositeTokens.burnComposite(compositeTokenId, owner);

        // Equip the composite item
    ...
    function _equipCompositeSkill(
        ...
        // Burn the composite skill token from the owner
        compositeTokens.burnComposite(compositeTokenId, owner);
    ...
    function unequip(
        ...
        if (tokenId > 10000) {
            ...
            _unequipCompositeToken(characterTokenId, msg.sender, tokenId);
        } else {
            if (tokenType == CharData.TokenType.Item) {
                _unequipItem(characterTokenId, msg.sender, tokenId);
            } else if (tokenType == CharData.TokenType.Skill) {
                _unequipSkill(characterTokenId, msg.sender, tokenId);
            } else {
                revert("Invalid token type");
            }
        }
    ...
    function _unequipSkill(
        ...
        for (uint256 i = 0; i < equippedSkills.length; i++) {
            if (equippedSkills[i] == skillId) {
                ...
                battleSkills.mintSkill(skillId, owner); // Minting the skill back to the owner
                return;
            }
        }
    ...
    function _unequipItem(
        ...
        // Check if the item tokenId is valid
        require(
            battleItems.totalSupply(itemTokenId) > 0,
            "Invalid item token ID"
        );
        ...
        battleItems.mintItem(itemTokenId, owner); // Minting the item back to the owner
    }
    ...
    function _unequipCompositeToken(
        ...
        if (tokenType == CharData.TokenType.CompositeItem) {
            _unequipCompositeItem(characterTokenId, owner, compositeTokenId);
        } else if (tokenType == CharData.TokenType.CompositeSkill) {
            _unequipCompositeSkill(characterTokenId, owner, compositeTokenId);
        } else {
    ...
    function _unequipCompositeItem(
        ...
        // Check if the composite token is valid
        require(
            compositeTokens.totalSupply(compositeTokenId) > 0,
            "Invalid composite token ID"
        );
        ...
        // Mint the composite token back to the owner
        compositeTokens.mintCompositeToken(compositeTokenId, owner);
    }
    ...
    function _unequipCompositeSkill(
        ...
        for (uint256 i = 0; i < equippedSkills.length; i++) {
            if (equippedSkills[i] == compositeTokenId) {
                ...
                // Mint the composite skill token back to the owner
                compositeTokens.mintCompositeToken(compositeTokenId, owner);
                return;
            }
        }
